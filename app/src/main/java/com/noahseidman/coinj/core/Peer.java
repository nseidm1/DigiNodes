...
import com.noahseidman.coinj.net.AbstractTimeoutHandler;
import com.noahseidman.coinj.store.BlockStore;
import com.noahseidman.coinj.store.BlockStoreException;
 ...
 * {@link com.noahseidman.coinj.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link com.noahseidman.coinj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
 * handshake completes.</p>
 */
public class Peer extends PeerSocketHandler {
    private static final Logger log = LoggerFactory.getLogger(Peer.class);

    protected static final ReentrantLock lock = Threading.lock("peer");
     ...
     * {@link com.noahseidman.coinj.net.NioClientManager#openConnection(java.net.SocketAddress, com.noahseidman.coinj.net.StreamParser)}
     * or
     * {@link com.noahseidman.coinj.net.NioClient#NioClient(java.net.SocketAddress, com.noahseidman.coinj.net.StreamParser, int)}.</p>
     *
    ...
    public void connectionClosed() {
        for (final PeerListenerRegistration registration : eventListeners) {
            registration.executor.execute(() -> registration.listener.onPeerDisconnected(Peer.this));
        }
    }
	...
	blockResponseTimeout.processMessage(m);
        ...
        if (m instanceof Ping) {
            ...
            for (final ListenerRegistration<PeerEventListener> registration : eventListeners) {
                registration.executor.execute(() -> registration.listener.onPeerConnected(Peer.this));
            }
...
//        }
    ...
    class BlockTimeoutHandler extends AbstractTimeoutHandler {
	    ...
	    public void processMessage(Message m) {
            setTimeoutEnabled(true); // No timeout until we are ready for the next message
    	}
     ...
     * Once the pong is received the value returned by {@link com.noahseidman.coinj.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link com.noahseidman.coinj.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link com.noahseidman.coinj.core.Peer#ping()} has never
...
//        }
    ...
    private void processAddressMessage(AddressMessage m) {
        for (final ListenerRegistration<PeerEventListener> registration : eventListeners) {
            registration.executor.execute(() -> registration.listener.onPeersDiscovered(this, m.getAddresses()));
        }
    }
