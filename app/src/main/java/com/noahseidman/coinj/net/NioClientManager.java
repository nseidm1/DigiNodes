...
public class NioClientManager extends AbstractExecutionThreadService implements ClientConnectionManager {
    ...
    class SocketChannelAndParser {
        ...
        SocketChannelAndParser(SocketChannel sc, StreamParser parser) {
            this.sc = sc; this.parser = parser;
        }
    }

    final Queue<SocketChannelAndParser> newConnectionChannels = new LinkedBlockingQueue<>();
        ...
        if (key.isValid() && key.isConnectable()) { // ie a client connection which has finished the initial connect process
            ...
            try {
                if (sc.finishConnect()) {
                    ...
                    handler.parser.connectionClosed(); // Failed to connect for some reason
                }
            } catch (Exception e) {
                ...
                handler.parser.connectionClosed();
            }
